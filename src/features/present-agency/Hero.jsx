...
export const Hero = () => {
  ...
  return (
    <section
      ...
      id="hero"
    >
      {/* VIDEO DESKTOP - Tech vector sin logo, loop infinito */}
      <video
        autoPlay
        loop
        muted
      ...
      >
        <source src="/hero-videos.mp4" type="video/mp4" />
      </video>

      {/* VIDEO MOBILE - Realista sin logo, loop infinito 9:16 */}
      <video
        autoPlay
        loop
        muted
        playsInline
        preload="auto"
        className="block md:hidden absolute inset-0 z-0 w-full h-full object-cover object-center"
      >
      ...
      <div className="h-full relative z-20 flex flex-wrap content-center justify-center px-5 sm:px-8 lg:px-12">
        <nav className="w-full lg:p-16 py-6 sm:py-8 lg:py-12 flex items-center justify-between gap-4 absolute top-0">
          <ul
            className={`absolute left-0 right-0 top-full mx-5 flex flex-col gap-5 overflow-hidden rounded-2xl border border-white/10 bg-patagonia-dark/90 p-5 shadow-2xl shadow-black/30 backdrop-blur-xl transition-all duration-500 ease-out lg:static lg:mx-0 lg:flex lg:max-w-[70%] lg:flex-row lg:gap-x-4 lg:gap-y-2 lg:overflow-visible lg:rounded-none lg:border-0 lg:bg-transparent lg:p-0 lg:shadow-none lg:backdrop-blur-none ${
              isMenuOpen
               ? 'visible max-h-96 translate-y-2 opacity-100 lg:translate-y-0'
                : 'invisible max-h-0 -translate-y-2 opacity-0 lg:visible lg:max-h-none lg:translate-y-0 lg:opacity-100'
            }`}
          >
            {heroNavItems.map((item) => (
              <li key={item.label} className="group">
                <a
                  href={item.link}
                  onClick={closeMenu}
                  className="block text-white hover:text-patagonia-teal font-semibold transition-all duration-300"
                >
                  {item.label}
                </a>
          ...
          <div className="flex m-1 px-6 w-full justify-between items-center gap-4 lg:w-auto lg:justify-end">
            ...
            <button
              ...
              className="flex h-11 w-11 items-center justify-center rounded-full border border-white/20 bg-black/20 text-white backdrop-blur-md transition-all duration-300 hover:border-patagonia-teal hover:text-patagonia-teal lg:hidden"
            >
              <span className="relative flex h-5 w-5 items-center justify-center">
                <FaBars
                  className={`absolute text-lg transition-all duration-300 ${isMenuOpen? 'rotate-90 scale-0 opacity-0' : 'rotate-0 scale-100 opacity-100'}`}
                />
                <FaXmark
                  className={`absolute text-xl transition-all duration-300 ${isMenuOpen? 'rotate-0 scale-100 opacity-100' : '-rotate-90 scale-0 opacity-0'}`}
                />
              </span>
            </button>
          </div>
        ...
        <div className="text-center">
          <h1 className="block text-white font-sans leading-none">
            <span className="mr-[2.5px] text-5xl sm:text-7xl lg:text-8xl text-patagonia-teal font-light">
              {'{'}
            </span>
            <span className="text-4xl sm:text-6xl lg:text-7xl font-bold">
              Patagonia<span className="text-patagonia-teal">Script</span>
            </span>
            <span className="ml-[2.5px] text-5xl sm:text-7xl lg:text-8xl text-patagonia-teal font-light">
              {'}'}
            </span>
          </h1>
